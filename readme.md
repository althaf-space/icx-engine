...
icx test benchmark                            # run the testing benchmark across the corpus; writes an HTML scorecard (ICX measured vs competitors published)
...
**Local engine.** Verification runs fully in-process and async - there is no external tester to install or keep running. ICX detects the right runners for each layer (unit/api/ui) and runs them on the repo-correct runtime, then reports one normalized result plus a Definition-of-Done confidence score. UI/agent tests replay headless by default; at the URL gate you can choose a visible browser and a per-step slowmo (default 1s when visible, 0 when headless) to watch each step.
...
**Comprehensive, self-verifying UI tests.** For UI/agent runs, ICX first drives a per-framework Element Census (React/Angular/Vue/Svelte) that enumerates and reconciles every interactive element, field, validation, and message on the screen - so authoring misses nothing (the count reconciliation makes "nothing missed" verifiable). It then converts that model into scenarios covering every functionality and every validation, and runs a live-DOM verify pass that resolves every selector against the real page and re-asks the agent to repair any that do not match - before scoring - so the run fires cleanly instead of misfiring on a stale selector.

**NL/ticket-driven scenarios.** In agent mode, ICX can author extra test scenarios from a plain-English intent (`nl_intent`) or a ticket's acceptance criteria (`acceptance_criteria`), both optional on `start_testing_session` - the connected editor agent authors and asserts a scenario for each, appended to the standard exploratory pass.
...
**Cross-browser and mobile.** `icx test benchmark` can run the same generated tests across Chromium, Firefox, and WebKit browser engines, plus mobile device profiles (via Chromium emulation), controlled by the `ICX_UI_TARGETS` environment variable (add `--install-browsers` to auto-approve installing a missing engine). This is a benchmark-only capability - `ICX_UI_TARGETS` is not read by the live/interactive `icx test` session.

**Self-healing selectors.** When the UI changes, ICX automatically repairs test selectors that become stale. The harness captures element fingerprints on the first run, then uses a deterministic score-matcher (no LLM) to find replacement selectors when a step misses. Healing rescues failing steps without changing passing ones, keeping tests green as the UI evolves.

**Visual regression.** ICX captures screenshot baselines on the first run and compares against them on later runs using pixel-diff (offline, no Percy or cloud). Mismatches are reported as test failures with the pixel-change percentage; re-baseline by deleting the flow's visual directory.

**Accessibility audits.** ICX runs full axe-core WCAG 2.1 AA accessibility audits offline, grouped by impact level (critical, serious, moderate, minor) with no external cloud service or network required.

**DB and network verification.** ICX verifies the database after a UI action by running a user-supplied command with the record value passed via environment variable (no string interpolation, credential-free). It checks graceful behavior under slow and offline network conditions in one flow. DB verification requires a user-supplied ICX_SQL_VERIFY_CMD; ICX never owns database credentials.

**Security testing (DAST + SAST + secrets + SCA, native, no installer).** Security is not a separate tool you install - the checks run inside every test run, self-hosted and deterministic. Runtime (DAST): each input gets reflected-XSS and DOM-XSS probes; each API endpoint gets 8 injection classes (SQLi/NoSQL/command/template/path/LDAP/XPath/CRLF), mass-assignment, broken-auth, object-level (IDOR-adjacent) and response-security-header checks. Static (over your own source): a secrets scan (cloud keys, private keys, tokens, hardcoded credentials - masked in the report), SAST-lite (real Python AST + cross-language rules for `eval`/`exec`, `shell=True`, disabled TLS, SQL string-concat, `innerHTML`/`dangerouslySetInnerHTML`, wildcard CORS, weak hashes and more), and SCA (dependency manifests flagged for unpinned versions and matched against an optional offline advisory file - `ICX_SCA_ADVISORY` or `.icx-advisories.json`). Findings are severity-graded and shown in a dedicated "Security scan" section of the HTML report. Honest by design: rule/AST matching (not full taint-flow) and offline/manifest dependency checks (not a live CVE feed).
