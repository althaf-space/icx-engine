...
| MCP tools | `icx_boost`, `icx_boost_refine`, `icx_skill_get`, `icx_skills_index`, `icx_draft_skill`, `icx_create_skill`, `jira_analyze_issue_fast`, `jira_analyze_issue`, `memory_search`, `memory_delete`, `memory_update`, 4 historical memory tools, `save_memory`, `icx_record_verification`, `reinforce_memory_usage`, `get_memory_audit`, 3 testing tools (`testing_start_session`, `testing_resume_session`, `testing_get_session_status`), 1 methodology tool (`icx_get_methodology`), 1 spec-lock tool (`icx_lock_plan`), 2 UI-auth tools (`icx_ui_auth_capture`, `icx_ui_auth_inline`), 2 discovery tools (`icx_find_tools`, `icx_call_tool` - only these plus a small core set of 6 are advertised in `tools/list`; the rest are reached through them, see "The MCP tools" section), 22 Sonar tools (`sonar_status`, `sonar_projects`, `sonar_branches`, `sonar_measures`, `sonar_quality_gate`, `sonar_findings`, `sonar_report`, `sonar_top_files`, `sonar_history`, `sonar_analyses`, `sonar_rule`, `sonar_rules`, `sonar_hotspot`, `sonar_source`, `sonar_metrics`, `sonar_quality_gate_definition`, `sonar_quality_profiles`, `sonar_issue_authors`, `sonar_issue_tags`, `sonar_issue_changelog`, `sonar_system_health`, `sonar_languages`), 42 git-workflow tools (`git_repo_status`, `git_start_branch`, `git_blame`, `git_log`, `git_show_commit`, `git_diff`, `git_diff_worktree`, `git_stage_and_commit`, `git_push`, `git_reverse_merge`, `git_get_conflict`, `git_read_file_at_ref`, `git_complete_resolution`, `git_adopt_resolution`, `git_discard_scratch`, `git_create_mr`, `git_finish_ticket`, `git_create_tag`, `git_delete_tag`, `git_retag`, `git_stash_create`, `git_stash_list`, `git_stash_apply`, `git_stash_pop`, `git_stash_drop`, `git_fetch`, `git_pull`, `git_sync`, `git_delete_branch`, `git_get_conflict_details`, `git_conflict_take_ours`, `git_conflict_take_theirs`, `git_conflict_apply_resolution`, `git_conflict_mark_resolved`, `git_conflict_abort`, `git_check_branch_name_policy`, `git_set_branch_policy`, `git_check_dependency_pins`, `git_restore_files`, `git_list_merged_branches`, `git_check_merge`, `git_repin_dependency`), 13 GitLab tools (`gitlab_list_merge_requests`, `gitlab_mr_changes`, `gitlab_list_commits`, `gitlab_compare`, `gitlab_list_tags`, `gitlab_list_branches`, `gitlab_list_pipelines`, `gitlab_pipeline_status`, `gitlab_job_log`, `gitlab_close_merge_request`, `gitlab_reopen_merge_request`, `gitlab_merge_merge_request`, `gitlab_refresh_merge_status`), 25 Workstatus tools (`workstatus_unread_notifications`, `workstatus_my_profile`, `workstatus_add_timesheet`, `workstatus_list_projects`, `workstatus_get_project`, `workstatus_project_budget_analytics`, `workstatus_list_tasks`, `workstatus_list_task_statuses`, `workstatus_list_milestones`, `workstatus_list_task_checklist`, `workstatus_list_members`, `workstatus_list_teams`, `workstatus_attendance_list`, `workstatus_attendance_stats`, `workstatus_list_timesheets`, `workstatus_list_timesheet_clients`, `workstatus_weekly_report`, `workstatus_timesheet_submission_kpis`, `workstatus_timesheet_submission_table`, `workstatus_list_expenses`, `workstatus_list_invoices`, `workstatus_payroll_report`, `workstatus_get_timesheet`, `workstatus_edit_timesheet`, `workstatus_recent_project_tasks`), 26 Jira write-back tools (`jira_get_close_requirements`, `jira_list_issue_types`, `jira_get_createmeta_fields`, `jira_apply_update`, `jira_create_issue`, `jira_delete_issue`, `jira_comment_list`, `jira_comment_add`, `jira_comment_edit`, `jira_comment_delete`, `jira_search`, `jira_get_issue`, `jira_link_types`, `jira_link_create`, `jira_link_delete`, `jira_set_assignee`, `jira_search_assignable_users`, `jira_attachment_upload`, `jira_attachment_delete`, `jira_get_current_user`, `jira_list_watchers`, `jira_list_worklogs`, `jira_set_watcher`, `jira_worklog_add`, `jira_worklog_edit`, `jira_worklog_delete`) (167 total) | Batch analysis, project-level summary |
...
```mermaid
flowchart TD
    A([Agent: files changed]) --> B[testing_start_session\nclassify + expand files]
...
icx workstatus add-time --project-id <id> --todo-id <id> --date <YYYY-MM-DD> \
    --from <full-datetime> --to <full-datetime> --duration <dur> --reason <text>   # add a REAL timesheet entry
...
icx workstatus edit-time --timesheet-id <id> --project-id <id> --todo-id <id> \
    --date <YYYY-MM-DD> --from <full-datetime> --to <full-datetime> --duration <dur> --reason <text> \
...
```sh
icx logs report                        # per-tool call count, error count, avg duration, token estimates for today
icx logs report --date 2026-08-25      # same, for a specific day
...
**`/icx-boost` - on demand, one call, two passes.** Boost is not something the agent calls on every message - it runs only when you (or your editor's MCP-prompt auto-surfacing) explicitly invoke `/icx-boost <your request>`. One call classifies the task, applies the mandatory ICX methodology, gathers only the codebase context the problem needs (graph/grep/memory - skipped for a plain question or when no repo is connected), and auto-refines the result into a CTO-grade prompt in the same call - no second tool call needed. For a work-tracker ticket, `jira_analyze_issue_fast` remains the ticket entrypoint, and that routing stays always-on (see above), independent of whether boost was invoked.
...
**Only 8 tools are advertised in `tools/list` - the other 159 are reached through 2 of those 8, not called directly.** This is deliberate: shipping all 167 tool definitions to every session costs ~54,800 tokens before any real work happens, and some editors (Windsurf) hard-cap total MCP tools at 100 across every connected server combined - ICX alone used to exceed that. The always-visible core: `git_repo_status`, `jira_analyze_issue_fast`, `jira_analyze_issue`, `sonar_status`, `icx_get_methodology`, `icx_boost`, plus two discovery tools:
...
| `icx_get_methodology` | Return the full ICX problem-solving methodology (intake, context, classify, decompose, plan, execute, self-check, confidence, fail-well, verify) with archetypes, decision rules, and pitfalls. `jira_analyze_issue`/`jira_analyze_issue_fast` already inject the mandatory one-pager into their response's `methodology` field - call this for the complete framework instead. No input. |
| `icx_lock_plan` | SPEC-LOCK - call after gathering context and deciding which files to change, BEFORE writing any code. Fuses graph + grep + semantic + memory signals and returns any HIGH-signal file your plan MISSED (a direct dependent, a co-change partner, a file a prior fix touched) - deterministic, no LLM. HARD RULE: do not write code until `ok` is true. Input: `issue_ref`, `chosen_files`, optional `justifications`/`project_path`/`keywords`. Returns `{ok, coverage, blocking_missed[], advisory_missed[], accepted[]}`. |
| `jira_analyze_issue_fast` | Always first - runs LLM analysis on text only, no attachment processing. Returns `work_item` (analysis + `image_paths` + `attachment_processing: "text_only"`), `memory`, and `graphs[]`. Timeout 45s. |
| `jira_analyze_issue` | Only when any of `work_item.analysis.pending_images`, `pending_audio`, or `pending_documents` is non-empty AND that media is relevant to the problem. Pass the same `project_paths` as the fast call. |
...
| `testing_start_session` | Begin the local AI testing loop for changed files (in-process polyglot runner suite - no external tester). |
| `testing_resume_session` | Continue at any human gate. At the `compat_scan` gate the agent assesses testability from first principles; every concern it finds is shown to the user at `compat_check`, who decides each one (apply change / drop / manual / accept-as-is). Verification runs locally via the runner plugins (unit/api/ui) on the repo-correct runtime. For UI/agent, a captured/reused login session (cookies + localStorage + sessionStorage) is restored before the flow runs, so the agent authors the test against the already-logged-in screen (no login steps re-authored). A gate that triggers real browser work (live-DOM verify/heal, scored execution) can return `{"status": "running"}` instead of a gate if it is still in progress past a short quick-timeout - poll `testing_get_session_status` rather than calling this tool again. |
| `testing_get_session_status` | Poll a session that returned `status: "running"`. Cheap, read-only. Returns `running` while the background gate is still executing, or the normal `{done, gate}` shape once it finishes. Input: `session_id`. |
| `icx_ui_auth_capture` | At the `auth_gate` when the user chose "capture" - opens a REAL headed browser for the user to log in by hand (NEVER ask for credentials in chat). Saves the authenticated session (cookies+localStorage) for this project+host once the user reaches `success_url` or closes the window; the UI test then replays already logged in. Input: `url`, `file_paths` (seed files, keys the session to this project), optional `success_url`. Returns `{ok, storage_state}` - resume the `auth_gate` with `{"auth_mode":"capture"}` after. |
| `icx_ui_auth_inline` | At the `auth_gate` when the user chose "inline" - the user supplies the application's own username/password and ICX drives the login form itself, then saves the session. Credentials reach only ICX's browser process, never stored beyond the resulting session. Input: `url`, `file_paths`, `username`, `password`, optional `success_url`/`user_selector`/`pass_selector`/`submit_selector`. Returns `{ok, storage_state}` - resume the `auth_gate` with `{"auth_mode":"inline"}` after. |
...
| `icx_record_verification` | Record Definition-of-Done evidence (exact command + output per check) before a ticket is done. Required for `save_memory` to record a verified success on the automated path, unless the fix was verified manually (`verified_by_human=true`). Returns `{accepted, missing, confidence}`. |
| `git_repo_status` | ALWAYS call first before any other `git_*` tool. Reports current branch, working-tree dirtiness, and leftover state from an interrupted prior run, plus rich structured status: `staged`/`unstaged` (each `{path, status}` with status one of modified/added/deleted/renamed/copied/type_changed/unmerged), `untracked`, `deleted` (union of staged+unstaged deletions), `renamed` (`{from, to}`), `conflicted`, `ahead`/`behind`, and `upstream` (null if none configured). If this repo has a confirmed `parent_branch`, also reports `commits_behind_parent` (how far `origin/<parent>` has moved since this branch's own merge-base with it - null if unconfirmed or never fetched locally) and `files_modified_upstream` - which of the currently dirty/staged/untracked files the parent branch ALSO touched since the branch point, a real silent-deletion risk if committed/merged as-is. git resolves the repo root upward automatically - call even if no `.git` is visible directly in `repo_path`'s own listing (e.g. a `ui/`/`svc/` subdirectory inside a larger repo is fine). Input: `repo_path`. |
| `git_start_branch` | Create a feature branch (or switch to it if it already exists - `switched_to_existing`/`created` report which). The branch name is always agent-proposed, human-approved - never invented silently. To switch to an EXISTING branch by its exact name without any derivation, use `git_checkout_branch` instead. Always fetches origin FIRST, every call, even if `parent_branch` was already confirmed earlier - a new branch is created off the real current tip of `origin/<parent_branch>`, never a possibly-stale local tracking ref. Name is `feature/<slug>-<ticket_key>` when `ticket_key` is given, or `feature/<slug>-<PROJECT_CODE>-0000` when it's null - `project_code` is then required; omitting it returns `status: needs_project_code` (ask the human, never invent one, no remembered default - asked fresh every time). Reports `commits_behind_parent` - nonzero on `switched_to_existing` means that EXISTING branch has fallen behind `origin/<parent_branch>` since it was created. NOT confirmation-gated - not destructive. If `parent_branch` is omitted and none is confirmed yet for this repo, returns `status: needs_confirmation`/`needs_manual_pick` the same way `git_reverse_merge` does - ask the human, then call again with `parent_branch` set. If this repo has `require_ticket_in_branch_name` enabled (see `git_set_branch_policy`, default off) and `ticket_key` is null, refuses BEFORE creating anything - the `-0000` placeholder never satisfies this policy, only a real ticket does. Input: `repo_path`, `summary_or_preferred_name`, optional `ticket_key` (null for a ticketless branch), `parent_branch`, `project_code` (required when `ticket_key` is null). |
| `git_checkout_branch` | Switch to a branch that ALREADY EXISTS, by its exact name - no derivation/slugify/prefix, unlike `git_start_branch`. Switches to a matching local branch, or fetches and creates a local tracking branch if it exists only on the remote; errors if found in neither place. A dirty working tree is auto-stashed first (never refused, never discarded) and left stashed - NOT auto-popped onto the target branch (could conflict against unrelated work); retrieve it with `git_stash_apply`/`git_stash_pop`. NOT confirmation-gated. Input: `repo_path`, `branch_name`, optional `remote` (default `origin`). |
...
| `git_repin_dependency` | Once `git_check_dependency_pins` reports a dependency BEHIND - resolves `target_ref` to that dependency's real current commit SHA the same way (local clone or matching GitLab connection), then rewrites ONLY that dependency's pinned ref/SHA in the manifest, byte-for-byte otherwise unchanged. Same manifest shapes as `git_check_dependency_pins`. CONFIRMATION-GATED: first call resolves the new SHA and returns `old_ref`/`new_ref` for the human to confirm. Does not stage/commit - follow with `git_stage_and_commit`. Input: `repo_path`, `manifest`, `dependency_name`, `target_ref`, optional `dep_repo_path`/`confirm_token`. |
| `git_check_merge` | Read-only "would this conflict" check between `source` and `target` - unlike `git_reverse_merge`/`git_pull`, never touches the working tree, the index, or either ref, so it can't be blocked by a permission classifier the way a real merge attempt can. Fetches origin first so a bare `target` branch name is checked against its real current remote tip. Returns `has_conflicts` plus `conflicting_files` (empty when clean). Read-only, UNGATED. Input: `repo_path`, optional `target` (defaults to the confirmed parent_branch)/`source` (defaults to the current branch). |
...
| `git_push` | Push the current branch to the remote without opening an MR yet - e.g. sharing progress with another developer on the same feature branch. Plain push only - no force, no rebase, no history rewrite. Refuses BEFORE issuing a token if the current branch violates this repo's branch-name policy (see `git_set_branch_policy`). A `GL-HOOK-ERR:` push rejection is translated into a plain-language server-policy message (with a specific tip for a `.gitignore` rejection) instead of the raw hook text. CONFIRMATION-GATED: the first call (no `confirm_token`) returns `pending_confirmation` plus a one-time token - show the exact branch and remote to the human and get explicit agreement before calling again with `confirm_token` set. `git_create_mr` already pushes automatically, so use this only when an MR should not be opened yet. Input: `repo_path`, optional `remote` (default `origin`), `confirm_token`. |
| `git_create_mr` | Create/reuse an MR for the current feature branch and attempt one immediate merge. Pushes the feature branch to origin first. Refuses BEFORE issuing a token if the source branch violates this repo's branch-name policy. A `GL-HOOK-ERR:` push rejection is translated into a plain-language server-policy message (with a specific tip for a `.gitignore` rejection) instead of the raw hook text. CONFIRMATION-GATED - the `pending_confirmation` payload shows both `source_branch` (the current feature branch, merges FROM) and `parent_branch` (the target, merges INTO); show both to the human, not just the target. `ticket_key` is nullable - the MR title is `ticket_summary` alone (no prefix) when null, never a manufactured ticket id. A refusal right after creation is polled (bounded, never indefinite) since GitLab computes mergeability asynchronously - response's `merge_status` is one of `MERGEABLE`/`CONFLICTED`/`CHECKING`/`BLOCKED`/`UNKNOWN`; when not merged, also returns `has_conflicts` (GitLab's own boolean) and `pipeline` (source branch's latest pipeline - id/status, plus `failed_job_name`/`failed_job_id` if it failed) so a real conflict, a blocked pipeline, and a transient check are never the same opaque refusal. Input: `repo_path`, `parent_branch`, `ticket_key`, `ticket_summary`, optional `max_poll_attempts`/`poll_delay_seconds`/`confirm_token`. |
...
| `git_list_merged_branches` | Discovery companion to `git_delete_branch` - lists every LOCAL branch fully merged into `target` (same ancestry check `git_delete_branch` itself uses - every result would delete with `unique_commits: 0`), each with its tip commit's author and date. Always excludes `target` itself and the current branch. `older_than_days` (default 0, no filter) restricts to branches whose tip is older than that. Local branches only - not a GitLab-wide view. Read-only, UNGATED. Input: `repo_path`, `target`, optional `older_than_days`. |
| `git_finish_ticket` | Post-merge cleanup - re-verifies the MR actually merged before doing anything. CONFIRMATION-GATED - the payload shows `feature_branch` (deleted locally AND on the GitLab remote after) and `parent_branch` (fast-forwarded locally); both backup tiers (`backup-latest/<key>` and any `backup/<key>-*` snapshots) are also removed. Cleanup is unconditional once the merge is verified, no opt-in flag - an already-gone remote branch (GitLab may delete it itself on merge) counts as success, not an error. `ticket_key` is nullable (used only for backup naming). Response includes `feature_branch_deleted`, `remote_branch_deleted`, `backup_latest_deleted`, `backups_deleted`. Input: `repo_path`, `parent_branch`, `feature_branch`, `ticket_key`, `mr_iid`, optional `confirm_token`. |
...
| `gitlab_close_merge_request` | USE WHEN an MR is stuck with a stale GitLab-side mergeability cache that local git and the pipeline both contradict - closing then `gitlab_reopen_merge_request` is the documented fix. Does not delete the MR or its branch. CONFIRMATION-GATED. Input: `mr_iid` (required), either `project` or `repo_path`, optional `confirm_token`. |
| `gitlab_reopen_merge_request` | Reopen a closed MR - most often right after `gitlab_close_merge_request`, to force GitLab to recompute a stale mergeability cache. CONFIRMATION-GATED. Input: `mr_iid` (required), either `project` or `repo_path`, optional `confirm_token`. |
| `gitlab_merge_merge_request` | Retry a merge on an MR that already exists (from `git_create_mr` or `gitlab_list_merge_requests`) - a standalone attempt that does NOT push, create, or touch local git. NOT confirm-gated by ICX itself - GitLab's own server-side protected-branch/approval rules are the real backstop, the same as any UNGATED read-only tool below relying on GitLab's own permissions. A refusal returns as a normal result (`merge_status` one of `CONFLICTED`/`BLOCKED`/`CHECKING`/`UNKNOWN`, plus pipeline detail), never raised. Input: `mr_iid` (required), either `project` or `repo_path`. |
| `gitlab_refresh_merge_status` | USE WHEN a push just landed on an MR's source branch and its mergeability needs re-checking without a full retry - re-fetches and reclassifies (same `MERGEABLE`/`CONFLICTED`/`CHECKING`/`BLOCKED`/`UNKNOWN` bucket everywhere else), never pushes or merges. Read-only, UNGATED. Input: `mr_iid` (required), either `project` or `repo_path`. |
...
| `jira_search` | Find Jira issues matching a JQL query. LIGHTWEIGHT, RAW - bare Jira issue fields, no LLM analysis; distinct from `jira_analyze_issue_fast`/`jira_analyze_issue`. UNGATED, read-only. Cost-capped server-side: `max_results` clamped to 100, `fields` defaults to `summary`/`status`/`issuetype` when omitted. Pagination is token-based (`next_page_token`/`page_token`). Input: `jql` (required), optional `fields`, `max_results`, `page_token`, `domain`. |
| `jira_get_issue` | A cheap, raw look at a single Jira issue's current fields - e.g. a quick status check before deciding an action. LIGHTWEIGHT, RAW - not a replacement for `jira_analyze_issue_fast`/`jira_analyze_issue`. UNGATED, read-only. Input: `issue_key` (required), optional `fields`. |
