...
class XAIProvider(LLMProvider):
    ...
    async def analyze(self, raw: RawIssueData) -> IssueContext:
        try:
            ...
            raise AuthError(
                "xAI API key is invalid or expired. Re-run `icx model --add` to update it."
            ) from exc
        except openai.RateLimitError as exc:
            raise RateLimited("xAI rate limit reached. Wait briefly and retry.") from exc
        except openai.APIConnectionError as exc:
            raise SourceUnavailable(
                "Cannot connect to xAI API. Check your network connection."
            ) from exc
