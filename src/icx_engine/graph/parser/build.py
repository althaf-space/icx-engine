...
def build_from_json(extraction: dict, *, directed: bool = False, root: str | Path | None = None) -> nx.Graph:
    ...
    for node in extraction.get("nodes", []):
        ...
        if "source" in node and "source_file" not in node:
            ...
            print(
                f"[icx graph] WARNING: node '{node_id}' uses field 'source' instead of "
        ...
        # icx graph versions that didn't always populate file_type) don't
    ...
    # Fill missing confidence field before validation.
    # LSP/framework resolvers emit confidence_score (float) without confidence (string enum).
    for _edge in extraction.get("edges", []):
        if not isinstance(_edge, dict):
            continue
        if _edge.get("confidence") in {"EXTRACTED", "INFERRED", "AMBIGUOUS"}:
            continue
        _score = _edge.get("confidence_score")
        if isinstance(_score, (int, float)):
            _edge["confidence"] = "EXTRACTED" if _score >= 0.8 else ("INFERRED" if _score >= 0.4 else "AMBIGUOUS")
        else:
            _edge["confidence"] = "EXTRACTED"

    errors = validate_extraction(extraction)
    # Dangling edges (stdlib/external imports) are expected - only warn about real schema errors.
    real_errors = [e for e in errors if "does not match any node id" not in e]
    if real_errors:
        print(f"[icx graph] Extraction warning ({len(real_errors)} issues): {real_errors[0]}", file=sys.stderr)
...
def deduplicate_by_label(nodes: list[dict], edges: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    print(f"[icx graph] Deduplicated {len(remap)} duplicate node(s) by label.", file=sys.stderr)
...
def build_merge(
    ...
    graph_path: str | Path = "icx-graph-out/graph.json",
    ...
    *,
    ...
    if prune_sources:
        ...
        if n_nodes:
            print(
                f"[icx graph] Pruned {n_nodes} node(s) from {n_files} deleted source file(s).",
                file=sys.stderr,
            )
        ...
        if edges_to_remove:
            ...
            print(
                f"[icx graph] Pruned {len(edges_to_remove)} edge(s) from deleted source file(s).",
                file=sys.stderr,
            )

        if not n_nodes and not edges_to_remove:
            print(
                f"[icx graph] {n_files} source file(s) deleted since last run - "
