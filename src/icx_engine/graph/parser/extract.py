...
def _make_id(*parts: str) -> str:
    # Preserves non-ASCII identifiers (NFKC normalized). Must stay in sync with build._normalize_id.
...
_JS_TREE_PARSERS: dict[str, object] = {}


def _get_js_parser(suffix: str):
    """Return a cached Parser for the given JS/TS file suffix.

    Language and Parser objects are expensive to construct (~30ms each on
    Windows). Caching them per-suffix reduces 2094-file symbol resolution
    from ~74s to ~2s on a typical React/TS project.
    """
    if suffix in _JS_TREE_PARSERS:
        return _JS_TREE_PARSERS[suffix]
    try:
        from tree_sitter import Language, Parser
        if suffix in (".ts", ".tsx"):
...
def _parse_js_tree(path: Path):
    try:
        parser = _get_js_parser(path.suffix)
        if parser is None:
            return None
        source = path.read_bytes()
...
def _pascal_resolve_unit(from_path: Path, unit_name: str) -> str:
    """Resolve a Pascal unit name to the icx graph node ID of its source file.
...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"icx graph requires tree-sitter >= 0.23.0 (Language API v2). "
...
def extract_bash(path: Path) -> dict:
    ...
    from icx_engine.graph.parser.security import sanitize_metadata  # module-level cached import
...
def _extract_sequential(
    ...
    for work_idx, (idx, path) in enumerate(uncached_work):
        ...
        if extractor is None:
            ...
            per_file[idx] = result
        if on_progress is not None and (work_idx + 1) % _CB_INTERVAL == 0:
            on_progress(done_offset + work_idx + 1, total_files)
    if on_progress is not None and uncached_work:
        on_progress(done_offset + len(uncached_work), total_files)
    if on_progress is None and total_files >= _PROGRESS_INTERVAL:
...
def extract(
    ...
    *,
    ...
    Args:
        ...
        cache_root: explicit root for icx-graph-out/cache/ (overrides the
    ...
    if on_progress is not None and done_count > 0:
        on_progress(done_count, total)

    if on_start is not None:
        on_start(len(uncached_work), total)
    ...
    if uncached_work:
        ...
        if not ran_parallel:
            _extract_sequential(uncached_work, per_file, effective_root, total, on_progress, done_count)
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from icx_engine.graph.parser.detect import _load_graphifyignore, _is_ignored, _is_noise_dir
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m icx_engine.graph.parser.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
