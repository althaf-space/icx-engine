...
# Output directory name - override with ICX_GRAPH_OUT env var for worktrees or
# shared-output setups. Accepts a relative name ("icx-graph-out-feature") or an
# absolute path ("/shared/graphify-out").
_GRAPHIFY_OUT = os.environ.get("ICX_GRAPH_OUT", "icx-graph-out")
...
# Use `icx graph extract --force` to bypass when needed.
...
def cache_dir(root: Path = Path("."), kind: str = "ast") -> Path:
    """Returns icx-graph-out/cache/{kind}/ - creates it if needed.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast") -> dict | None:
    ...
    Cache value: stored as icx-graph-out/cache/{kind}/{hash}.json
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast") -> None:
    ...
    Stores as icx-graph-out/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
