...
def _ssrf_guarded_socket():
    ...
    Not thread-safe, but icx-graph is a single-threaded CLI tool.
    """
...
def validate_graph_path(path: str | Path, base: Path | None = None) -> Path:
    ...
    *base* defaults to the `icx-graph-out` directory relative to CWD.
    Also requires the base directory to exist, so a caller cannot
    trick icx-graph into reading files before any graph has been built.
    ...
    if base is None:
        ...
        for candidate in [resolved_hint, *resolved_hint.parents]:
            if candidate.name == "icx-graph-out":
                base = candidate
                break
        if base is None:
            base = Path("icx-graph-out").resolve()
    ...
    try:
        ...
        raise ValueError(
            ...
            "Only paths inside icx-graph-out/ are permitted."
        )
