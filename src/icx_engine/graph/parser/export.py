...
from icx_engine.graph.parser.security import sanitize_label
from icx_engine.graph.parser.analyze import _node_community_map
from icx_engine.graph.parser.build import edge_data
...
_BACKUP_ARTIFACTS = [
    "graph.json",
    "GRAPH_REPORT.md",
    ".icx_graph_labels.json",
    ".icx_graph_analysis.json",
    "manifest.json",
    ".icx_graph_semantic_marker",
    "cost.json",
]
...
def backup_if_protected(out_dir: Path) -> "Path | None":
    ...
    - .icx_graph_semantic_marker is present (graph cost real LLM tokens), or
    - .icx_graph_labels.json contains at least one non-default community label
    ...
    is_semantic = (out / ".icx_graph_semantic_marker").exists()
    is_curated = False
    labels_file = out / ".icx_graph_labels.json"
...
_ROLE_BORDER_COLORS: dict[str, str] = {
    "[controller]": "#E15759",
    "[service]":    "#4E79A7",
    "[dao]":        "#B07AA1",
    "[model]":      "#EDC948",
...
function showInfo(nodeId) {{
  ...
  document.getElementById('info-content').innerHTML = `
    ...
    ${{n._role_tag ? `<div class="field">Role: ${{esc(n._role_tag)}}</div>` : ''}}
...
def _resolve_resolver_tag(link: dict) -> str:
    """Derive a resolver_tag for edges that have no confidence_source."""
    cs = link.get("confidence_source", "")
    if cs:
        return cs
    et = link.get("edge_type") or link.get("relation", "")
    sc = link.get("confidence_score")
    try:
        sc = float(sc) if sc is not None else 1.0
    except (TypeError, ValueError):
        sc = 1.0
    if et == "contains":
        return "ast_structural"
    if sc >= 0.95:
        return "ast_direct"
    if sc >= 0.8:
        return "ast_inferred"
...
def to_json(
    ...
    *,
    ...
    for link in data["links"]:
        ...
        if "resolver_tag" not in link:
            link["resolver_tag"] = _resolve_resolver_tag(link)
        rel = (link.get("relation") or "").lower()
        link["edge_type"] = _RELATION_TO_TYPE.get(rel, rel or "relates")
    from icx_engine.graph.parser.roles import role_tag as _role_tag_fn
    for node in data["nodes"]:
        node["role_tag"] = _role_tag_fn(node.get("source_file") or "")
...
def to_html(
    ...
    for node_id, data in G.nodes(data=True):
        ...
        border_color = _ROLE_BORDER_COLORS.get(role, color)
        ...
        vis_nodes.append({
            "id": node_id,
            "label": label,
            "color": {"background": color, "border": border_color, "highlight": {"background": "#ffffff", "border": border_color}},
            "size": round(size, 1),
    ...
    for u, v, data in G.edges(data=True):
        ...
        cs = data.get("confidence_score", 1.0 if confidence == "EXTRACTED" else 0.5)
        relation = data.get("relation", "")
        edge_type = data.get("edge_type", _RELATION_TO_TYPE.get(relation.lower(), relation or "relates"))
        resolver_tag = data.get("resolver_tag", data.get("confidence_source", ""))
        true_src = data.get("_src", u)
        true_tgt = data.get("_tgt", v)
        edge_color = _EDGE_TYPE_COLORS.get(edge_type, _EDGE_TYPE_COLORS["relates"])
        opacity = round(0.15 + 0.85 * cs, 3)
        vis_edges.append({
            "from": true_src,
            "to": true_tgt,
            "label": relation,
            "title": _html.escape(f"{edge_type} [{resolver_tag}] conf:{cs:.2f}"),
            "dashes": cs < 0.75,
            "width": max(1, round(1 + 2 * cs)),
...
def to_context_json(
    ...
    """Write a token-budgeted context slice around focus_node_id to output_path.

    Performs BFS from focus_node_id (outbound then inbound edges) up to depth
    hops, filtering edges below min_confidence. Fills nodes greedily until the
    tiktoken token budget is reached, then writes a JSON file with the selected
    subgraph. Returns the number of tokens used.

    The output JSON has keys: focus_node, nodes, edges, token_budget, tokens_used.
    """
    try:
    ...
    if focus_node_id not in G:
        raise ValueError(f"focus_node_id {focus_node_id!r} not in graph")

    visited: list[str] = []
    frontier = [focus_node_id]
    seen: set[str] = {focus_node_id}
    for _ in range(depth):
        ...
        for node in frontier:
            for nb in G.neighbors(node):
                ...
                cs = edge_data.get("confidence_score", 0.55)
    ...
    for u, v, edata in G.edges(data=True):
        if u not in selected_ids or v not in selected_ids:
            continue
        cs = edata.get("confidence_score", 0.55)
        if cs < min_confidence:
            continue
        edge_obj = {"source": u, "target": v, **edata}
        t = _count(edge_obj)
        if tokens_used + t > token_budget:
            break
        selected_edges.append(edge_obj)
        tokens_used += t
    ...
    Path(output_path).write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")  # nosec
    return tokens_used
