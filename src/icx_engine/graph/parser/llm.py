...
# Gemini, and OpenAI.
# Used by `icx graph extract . --backend gemini` and the benchmark scripts.
# The default icx graph pipeline uses Claude Code subagents via skill.md;
...
_EXTRACTION_SYSTEM = """\
You are an icx graph semantic extraction agent. Extract a knowledge graph fragment from the files provided.
...
If the user message starts with an "Available cross-file edge targets" block,
cross-file edges may only reference symbols listed there. Do not invent
target names that are not present in that block; omit the edge instead.
...
def _read_files(
    paths: list[Path],
    root: Path,
    symbol_table: dict[str, list[str]] | None = None,
) -> str:
    """Return file contents formatted for the extraction prompt.

    When `symbol_table` is provided, a compact "Available targets" header is
    prepended listing the exported symbols per project file. The LLM is
    instructed to only reference these symbols when emitting cross-file
    edges, which sharply reduces hallucinated target names.
    """
    parts: list[str] = []
    for p in paths:
        try:
    ...
    header = _format_symbol_table_header(paths, root, symbol_table)
    if not header:
        return body
    return f"{header}\n\n{body}"


def _format_symbol_table_header(
    chunk_paths: list[Path],
    root: Path,
    symbol_table: dict[str, list[str]],
) -> str:
    """Render the "Available targets" block. Lists symbols for files in this
    chunk first, then a compact directory of every other project file the
    chunk might cite. Keeps token cost low by truncating per-file lists
    when an export count blows past a small cap.
    """
    chunk_rels: set[str] = set()
    for p in chunk_paths:
        try:
            chunk_rels.add(p.relative_to(root).as_posix())
        except ValueError:
            continue
    ...
    lines: list[str] = [
        "Available cross-file edge targets",
        "(only reference symbols below; do not invent new names):",
    ]

    in_chunk = sorted(rel for rel in symbol_table if rel in chunk_rels)
    other = sorted(rel for rel in symbol_table if rel not in chunk_rels)

    if in_chunk:
        lines.append("# Files in this chunk")
        for rel in in_chunk:
            lines.append(_format_symbol_line(rel, symbol_table[rel]))

    if other:
        lines.append("# Other project files")
        for rel in other:
            lines.append(_format_symbol_line(rel, symbol_table[rel]))
...
def _format_symbol_line(rel: str, symbols: list[str]) -> str:
    cap = 30
    if len(symbols) > cap:
        shown = ", ".join(symbols[:cap])
        return f"  {rel}: {shown} ... (+{len(symbols) - cap} more)"
    return f"  {rel}: {', '.join(symbols)}"


def build_symbol_table_from_ast(ast_extraction: dict, root: Path) -> dict[str, list[str]]:
    """Group AST node labels by their containing file's relative path.

    Pure function so it can be precomputed once and passed into LLM chunk
    extraction without pickling closures. Symbols are normalised the same
    way the resolvers normalise them (strip parens + leading method dot)
    so the LLM and the AST node-id space agree on names.
    """
    root_str = str(root).replace("\\", "/")
    out: dict[str, list[str]] = {}
    seen_per_file: dict[str, set[str]] = {}

    for node in ast_extraction.get("nodes", []):
        src = (node.get("source_file") or "").replace("\\", "/").strip()
        label = (node.get("label") or "").strip()
        if not src or not label:
            continue
        if src.startswith(root_str + "/"):
            rel = src[len(root_str) + 1 :]
        elif src.startswith(root_str):
            rel = src[len(root_str) :].lstrip("/")
        else:
            continue
        ...
        if not symbol or symbol.endswith(".py"):
            continue

        bucket = seen_per_file.setdefault(rel, set())
        if symbol in bucket:
            continue
        bucket.add(symbol)
        out.setdefault(rel, []).append(symbol)
...
def _parse_llm_json(raw: str) -> dict:
    ...
    if len(raw) > _LLM_JSON_MAX_BYTES:
        print(
            f"[icx graph] LLM response exceeds {_LLM_JSON_MAX_BYTES} bytes "
    ...
    try:
        ...
        print(f"[icx graph] LLM returned invalid JSON, skipping chunk: {exc}", file=sys.stderr)
        return {"nodes": [], "edges": [], "hyperedges": []}
...
def _call_openai_compat(
    ...
    *,
    ...
    if backend == "ollama":
        ...
        if num_ctx_raw:
            try:
                ...
                print(
                    f"[icx graph] GRAPHIFY_OLLAMA_NUM_CTX={num_ctx_raw!r} is not a valid integer; "
                ...
                if num_ctx < estimated_input:
                    print(
                        f"[icx graph] warning: GRAPHIFY_OLLAMA_NUM_CTX={num_ctx} is smaller than "
    ...
    if _response_is_hollow(raw_content, result) and result["finish_reason"] != "length":
        print(
            f"[icx graph] {backend or 'backend'} returned a hollow response "
    ...
    if output_tokens < 50 and backend == "ollama":
        print(
            "[icx graph] warning: ollama returned very few tokens - likely causes: "
...
def _call_claude(api_key: str, model: str, user_message: str, max_tokens: int = 8192) -> dict:
    ...
    if _response_is_hollow(raw_content, result) and result["finish_reason"] != "length":
        print(
            "[icx graph] claude returned a hollow response; treating as "
...
def _call_claude_cli(user_message: str, max_tokens: int = 8192) -> dict:
    ...
    a pay-as-you-go API key just to run icx graph's semantic pass.
    """
    import shutil
    import subprocess
    ...
    if _response_is_hollow(raw_content, result) and result["finish_reason"] != "length":
        print(
            "[icx graph] claude-cli returned a hollow response; treating as "
            "truncation so adaptive retry can bisect the chunk.",
            file=sys.stderr,
        )
        result["finish_reason"] = "length"
...
def _call_bedrock(model: str, user_message: str, max_tokens: int = 8192) -> dict:
    ...
    try:
        ...
        raise ImportError(
            "AWS Bedrock extraction requires boto3. Run: pip install icx-engine"
        ) from exc
    ...
    if _response_is_hollow(text, result) and result["finish_reason"] != "length":
        print(
            "[icx graph] bedrock returned a hollow response; treating as "
            "truncation so adaptive retry can bisect the chunk.",
            file=sys.stderr,
        )
        result["finish_reason"] = "length"
...
def extract_files_direct(
    ...
    if not key and backend == "ollama":
        ...
        print(
            "[icx graph] WARNING: ollama backend selected with no OLLAMA_API_KEY set; "
    ...
    user_msg = _read_files(files, root, symbol_table=symbol_table)
...
def _extract_with_adaptive_retry(
    ...
    try:
        ...
        if len(chunk) <= 1:
            print(
                f"[icx graph] single-file chunk {chunk[0]} exceeds model context "
        ...
        if _depth >= max_depth:
            print(
                f"[icx graph] chunk of {len(chunk)} still overflows context at "
                f"recursion depth {_depth} (max {max_depth}) - dropping",
                file=sys.stderr,
            )
            return {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0, "model": model, "finish_reason": "stop"}
        print(
            f"[icx graph] chunk of {len(chunk)} exceeded context at depth "
    ...
    if len(chunk) <= 1:
        print(
            f"[icx graph] single-file chunk {chunk[0]} truncated at "
    ...
    if _depth >= max_depth:
        print(
            f"[icx graph] chunk of {len(chunk)} still truncated at recursion "
    ...
    print(
        f"[icx graph] chunk of {len(chunk)} truncated at depth {_depth}, "
...
def extract_corpus_parallel(
    ...
    if merged["failed_chunks"] > 0:
        print(
            f"[icx graph] WARNING: {merged['failed_chunks']}/{total} semantic chunk(s) failed"
...
def extract_corpus_two_pass(
    ...
    *,
    ...
    max_retry_depth: int = 3,
    symbol_table: dict[str, list[str]] | None = None,
    keep_single_pass: bool = True,
    rng_seed: int = 0xC0FFEE,
) -> dict:
    """Run extract_corpus_parallel twice and intersect the edge sets.

    Pass 1 uses the natural file order; pass 2 shuffles with a fixed seed
    so chunk boundaries differ. Edges that appear in both passes are tagged
    `LLM_CONSENSUS` confidence; edges seen in only one pass are tagged
    `LLM_SINGLE_PASS`. When `keep_single_pass=False`, single-pass edges
    are dropped entirely (precision over recall).

    Nodes from both passes are unioned by ID. Hyperedges are concatenated.

    Doubles LLM cost. Use only when the LLM tier is required; pure AST +
    LSP + framework resolvers already deliver high recall and do not need
    consensus filtering.
    """
    import random
    from icx_engine.graph.parser.confidence import (
        LLM_CONSENSUS, LLM_SINGLE_PASS, clamp,
    )

    pass1 = extract_corpus_parallel(
        files,
        backend=backend, api_key=api_key, model=model, root=root,
        token_budget=token_budget, max_concurrency=max_concurrency,
        max_retry_depth=max_retry_depth, symbol_table=symbol_table,
    )

    shuffled = list(files)
    random.Random(rng_seed).shuffle(shuffled)
    pass2 = extract_corpus_parallel(
        shuffled,
        backend=backend, api_key=api_key, model=model, root=root,
        token_budget=token_budget, max_concurrency=max_concurrency,
        max_retry_depth=max_retry_depth, symbol_table=symbol_table,
    ...
    def _edge_key(e: dict) -> tuple[str, str, str]:
        return (
            str(e.get("source", "")),
            str(e.get("target", "")),
            str(e.get("relation", "")).lower(),
        )

    edges1 = pass1.get("edges", [])
    edges2_keys = {_edge_key(e) for e in pass2.get("edges", [])}
    ...
    for edge in edges1:
        ...
        if key in seen_keys:
            continue
        seen_keys.add(key)
        tier = LLM_CONSENSUS if key in edges2_keys else LLM_SINGLE_PASS
        edge["confidence_score"] = clamp(tier)
        edge["confidence_source"] = (
            "llm_consensus" if tier == LLM_CONSENSUS else "llm_single_pass"
        )
        if tier == LLM_CONSENSUS:
            consensus.append(edge)
        else:
            single.append(edge)

    for edge in pass2.get("edges", []):
        key = _edge_key(edge)
        if key in seen_keys:
            continue
        seen_keys.add(key)
        edge["confidence_score"] = clamp(LLM_SINGLE_PASS)
        edge["confidence_source"] = "llm_single_pass"
        single.append(edge)

    kept_edges = consensus + (single if keep_single_pass else [])

    nodes_by_id: dict[str, dict] = {}
    for node in pass1.get("nodes", []) + pass2.get("nodes", []):
        nid = node.get("id")
    ...
    return {
        ...
        "edges": kept_edges,
        "hyperedges": list(pass1.get("hyperedges", []) + pass2.get("hyperedges", [])),
...
def _validate_ollama_base_url(url: str) -> None:
    ...
    try:
        ...
        print(
            f"[icx graph] WARNING: OLLAMA_BASE_URL={url!r} is not a parseable URL.",
            file=sys.stderr,
        )
        return
    if parsed.scheme not in ("http", "https"):
        print(
            f"[icx graph] WARNING: OLLAMA_BASE_URL has unexpected scheme {parsed.scheme!r}; "
    ...
    if not is_loopback:
        ...
        print(
            f"[icx graph] WARNING: OLLAMA_BASE_URL points to non-loopback host {host!r}{scheme_note}. "
