...
from icx_engine.graph.parser.google_workspace import (
...
_MANIFEST_PATH = "icx-graph-out/manifest.json"
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the icx graph extract pipeline.
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    "icx-graph-out",  # never treat own output as source input (#524)
    ...
    ".icx_graph",  # icx graph's own extraction cache - never index self-generated data
...
def _parse_gitignore_line(raw: str) -> str:
    """Parse one raw line from a .icxgraphignore file per gitignore spec.
...
def _load_graphifyignore(root: Path) -> list[tuple[Path, str]]:
    """Read .icxgraphignore files and return (anchor_dir, pattern) pairs.
    ...
    for d in dirs:
        # Prefer .icxgraphignore; fall back to .gitignore so projects that already
        # maintain a .gitignore get sensible defaults without duplicating it (#945).
        ignore_file = d / ".icxgraphignore"
...
def _is_ignored(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if the path should be ignored per .icxgraphignore patterns.
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None) -> dict:
    ...
    # Load per-project .icxignore from ~/.icx/graphs/<project_id>/.icxignore.
    # Patterns are appended after .icxgraphignore so they take precedence.
    try:
        from icx_engine.graph import storage as _storage
        from icx_engine.graph.parser.icxignore import _parse_patterns as _icx_parse
        _pid = _storage.derive_project_id(root)
        _ipath = _storage.icxignore_path(_pid)
        if _ipath.exists():
            for _pat in _icx_parse(_ipath.read_text(encoding="utf-8")):
                _clean = _pat.rstrip("/")
                if _clean:
                    ignore_patterns.append((root, _clean))
    except Exception:
        pass

    # CLI --exclude patterns are anchored at the scan root and appended last
    # so they win over any .icxgraphignore/.gitignore rules (#947).
    if extra_excludes:
    ...
    converted_dir = root / "icx-graph-out" / "converted"
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install icx-engine]")
                continue
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      - written by `icx-graph update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" - written by `icx graph extract` after semantic extraction.
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `icx graph extract` so that
        files touched by `icx-graph update` (AST-only) are re-extracted
        semantically.
