...
|   +-- boost/                   # universal boost channel: classify.py (archetype), router.py (adaptive signal activation), brief.py (deterministic boosted brief + prompt), benchmark/ (proof: corpus+grader+runner+HTML scorecard), mcp_tools.py (icx_boost/icx_boost_refine tool surface)
...
|   +-- graph/                  # codebase knowledge graph; graph/mcp_tools.py owns the graph_* tool surface (Tool() defs + dispatch_graph_tool())
...
|   +-- testing/                # Testing orchestration module (local engine); testing/mcp_tools.py owns the testing-session + ui_auth_* tool surface
|   |   +-- __init__.py
...
|   +-- memory/                 # local LanceDB + ONNX memory (see section 7); memory/mcp_tools.py owns the memory_* tool surface
|   +-- skills/                 # learned-skill Markdown store (see section 7b); skills/mcp_tools.py owns the icx_get_methodology/icx_skill_*/icx_draft_skill/icx_create_skill tool surface
...
|   +-- sonar/                  # SonarQube code-quality reader (client/service + mcp_tools.py owning the sonar_* tool surface)
...
|   |   |                       # --project-code, wraps start_branch; --project-code required and interactively
|   |   |                       # prompted via _resolve_project_code_or_ask when --ticket is omitted, no remembered
|   |   |                       # default), `icx git checkout <BRANCH_NAME>` (--remote, wraps checkout_branch - exact
|   |   |                       # name, no derivation, auto-stashes a dirty tree), `icx git sync`, `icx git push`
|   |   |                       # (--remote, plain push, prompts via typer.confirm before pushing), `icx git mr`,
|   |   |                       # `icx git finish` (now always removes the feature branch locally AND on GitLab plus
|   |   |                       # both backup tiers - no --delete-backups flag anymore, cleanup is unconditional once
|   |   |                       # the merge is verified), `icx git tag`, `icx git blame <FILE>` (--from-line/--to-line),
|   |   |                       # `icx git log` (--file/--author/--since/--limit), `icx git show <SHA>`,
|   |   |                       # `icx git diff <REF_A> <REF_B>`
...
|   |                           # NOT confirmation-gated - branch name is agent-proposed/human-approved by convention,
|   |                           # not by a token; ticketless names now require project_code (returns needs_project_code
|   |                           # if omitted, asked fresh every call, no remembered default unlike parent_branch) and
|   |                           # produce feature/<slug>-<PROJECT_CODE>-0000 instead of a bare feature/<slug>)/
|   |                           # git_checkout_branch (NEW - wraps checkout_branch: switches to an EXISTING branch by
|   |                           # its exact name, no derive/slugify/prefix; auto-stashes a dirty tree rather than
|   |                           # refusing, leaves the stash in place - never auto-popped onto the target branch;
|   |                           # errors if the branch exists neither locally nor on the remote; NOT confirmation-gated)/
|   |                           # git_blame/git_log/git_show_commit/git_diff/git_diff_worktree
...
|   |                           # finish_ticket (ticket_key nullable, same pattern; post_merge_cleanup is now `async
|   |                           # def` - awaits the GitLab merge-check directly instead of asyncio.run(), which raised
|   |                           # inside the MCP server's own running event loop; cleanup is now unconditional once
|   |                           # the merge is verified - deletes the feature branch locally AND on the GitLab remote
|   |                           # (idempotent: an already-gone remote branch, e.g. GitLab's own remove_source_branch,
|   |                           # is treated as success) plus BOTH backup tiers (backup-latest/<key> via the new
|   |                           # safety.delete_backup_latest(), and any backup/<key>-* snapshots via
|   |                           # prune_old_backups(keep=0)) - no delete_backups opt-in flag anymore)/
|   |                           # create_tag (now validates BOTH the
...
|   |   |                       # from the read pipeline and from jira_analyze_issue_fast/jira_analyze_issue); link_types/create_link/
...
|   |                           # distinguish themselves from jira_analyze_issue_fast/jira_analyze_issue); jira_link_types/
...
Every `icx analyze` call and every `jira_analyze_issue_fast` / `jira_analyze_issue` MCP call runs through `engine.run()`:
...
The CLI exposes this as `--profile NAME` on `icx analyze`. The MCP server exposes it as an optional `profile` property in the `jira_analyze_issue_fast` and `jira_analyze_issue` tool schemas. Both validate the value and pass it through as `profile_override`.
...
| # | Tool | Purpose |
|---|------|---------|
| 1 | `jira_analyze_issue_fast` | Text-only analysis - always call first |
| 1 | `jira_analyze_issue` | Full vision analysis - call only when images needed |
...
| 20 | `icx_record_verification` | Record Definition-of-Done evidence (command + output per check) before done |
| - | `icx_lock_plan` | SPEC-LOCK before coding: submit files you will change; fuses graph+grep+semantic+memory, returns HIGH-signal files missed. Agent must not code until `ok` (include or justify each miss). Pure/deterministic, no LLM. `context_completeness.py`. |
| 21 | `testing_start_session` | Begin the local testing session for confirmed files |
| 22 | `testing_resume_session` | Advance the testing session at each human gate. A gate that triggers real browser work (the agent authoring/running/self-healing its own Playwright test) may return `{"status": "running"}` instead of blocking - see `testing_get_session_status`. |
| 23 | `testing_get_session_status` | Poll a session whose last call returned `status: "running"`. Cheap, read-only, safe to call repeatedly. |

`jira_analyze_issue_fast` and `jira_analyze_issue` both call `_handle_analyze_issue()` internally - the only difference is `skip_vision=True` vs `skip_vision=False`. The `_call_tool()` dispatcher sets this based on which tool name was called.
...
the testing-session state machine (`mcp_server.py`'s `testing_start_session`/
`testing_resume_session` gate flow), deserving its own scoped design proposal
...
| `building` | User-initiated build in progress; proceed now with grep/glob; optionally re-call `jira_analyze_issue_fast` when ETA elapses to cross-check file selection |
...
**Async job/poll pattern (opacity fix):** a gate that triggers real browser work (the agent authoring, running, and self-healing its own Playwright test at `author_flow`) can take minutes; the naive `await graph.ainvoke(...)` blocking the whole MCP call made a legitimate multi-minute wait indistinguishable from a hang. `testing_start_session`/`testing_resume_session` now run `graph.ainvoke` as a tracked `asyncio.Task` (`_testing_invoke_tracked` in `mcp_server.py`) and `wait_for` it with a short quick-timeout (`_TESTING_QUICK_TIMEOUT`, 20s). A gate that answers within that window behaves exactly as before (inline `{done, gate}` result, zero contract change for the ~90% of gates that are instant human-in-loop round trips). A gate still running past the timeout returns `{"status": "running", "done": false, "gate": null}` immediately - the task keeps executing in the background (`asyncio.shield` means the `wait_for` timeout never cancels it) - and the caller polls the new `testing_get_session_status(session_id)` tool instead. The task is tracked in module-level `_TESTING_RUNNING`/`_TESTING_ERRORS` dicts keyed by `session_id`; a `testing_resume_session` call while a task is still running for that session is rejected (points the caller at the status tool) rather than double-invoking `ainvoke` on the same LangGraph thread. Best-effort tracking only: an MCP server restart drops `_TESTING_RUNNING`, but `testing_get_session_status` still falls back to a plain `graph.aget_state` read in that case.
...
`testing_start_session` optionally takes `nl_intent` (a plain-English scenario request, e.g. "test
...
### MCP tool: icx_create_skill

The memory-free creation path - use this when the user directly asks for a general-purpose skill rather than following up a verified fix. Unlike `icx_draft_skill`, it has zero `issue_key`/`MemoryManager` dependency.
...
`save_memory`'s own MCP schema carries no skill-related fields at all - skill authoring is fully decoupled into the separate `icx_draft_skill` tool call described above. The only thing `save_memory` itself does that touches skills is compute the optional `related_skills` hint in its response.
...
| Tool | Skill attached |
|---|---|
| `testing_start_session` | `testing-session-driver` |
| `sonar_status` | `sonar-quality-review` |
| `jira_analyze_issue_fast` / `jira_analyze_issue` | `ticket-context-analysis` |
...
| `git/naming.py` ticketless `-0000` suffix / `git/policy.py:validate_branch_name` | The literal `0000` segment is ICX's ticketless placeholder, not a real ticket number - `validate_branch_name` deliberately treats a trailing `-0000` as NOT satisfying `require_ticket_in_branch_name`. Changing either side without the other silently reopens the bypass this closes (a repo that opted into requiring real tickets would otherwise accept the placeholder). |
| `testing_resume_session`/`testing_start_session`/`jira_analyze_issue_fast` tool descriptions | Deliberate exceptions to the 2048-char tool-description ceiling (`tests/test_mcp.py::test_no_new_tool_exceeds_description_length_ceiling`'s `_DESCRIPTION_LENGTH_EXCEPTIONS`). Each carries dense, incident-derived instruction text pinned word-for-word by regression tests (banned rationalization phrases, exact gate-shape keywords, escape-hatch names, mandatory next-tool-call rules) - a 2025 attempt to shrink them via automated trimming silently regressed 7 of those tests before being caught and reverted. Shrinking these safely requires a human who understands the testing-gate state machine and the ticket-analysis pipeline redesigning the information architecture on purpose (e.g. having the tool's own response carry only the CURRENT gate's instructions instead of all gates every call) - not a mechanical trim pass. `save_memory` was in the same size range but had NO test locks on its text, so it was safely trimmed to 2038 chars instead. `jira_analyze_issue` (the non-fast variant) was also removed from this list: it was 95.5% byte-identical duplicate text of `jira_analyze_issue_fast` (which is ALWAYS called first in a session) - deduplicating down to a short pointer at `jira_analyze_issue_fast` cut it from 13,149 to 1,688 chars with zero content loss, since the agent has already seen the full rulebook by the time this tool is ever relevant. Don't reflexively assume every oversized description needs this same "point at another tool" fix - it only works when one variant is provably always-called-before the other in the same session; `jira_analyze_issue_fast` itself has nothing earlier to point at, hence it stays an exception. |
